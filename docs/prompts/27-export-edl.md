...
当自动剪辑方案已经确定，但还需要交给专业剪辑软件继续调色、混音、精剪或给协作剪辑师复核时，用 `scripts/export_edl.py` 或 `scripts/export_fcpxml.py` 把本项目的 `render_config.json` 或 rough/jump cut list 导出成 NLE 时间线。
...
- 当前实现是单视频轨 EDL / 单 spine FCPXML，适合选段、粗剪、调色/混音交接；复杂 overlay、字幕、章节卡和 B-roll 仍以 `render_final.py` / `export_capcut.py` 为准。
...
- `*.edl.json` / `*.fcpxml.json` manifest 是审计依据：如果 NLE 对 XML/EDL 处理不一致，以 manifest 里的秒数和绝对路径为准。
