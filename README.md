...
`publish_ready` 默认要求 `transcript` / `clean_script` / `render_config` / `master_video` / `render_qa` / `caption` 都存在；如果发现 `storyboard_assets.json`、`provider_decision.json`、`transition_bridge_plan.json`、`motion_guard.json` 或 `speaker_turns.json` 里仍有 blocking、approval_required、budget_blocked、QA fail 等问题，`--strict` 返回 2。需要把字幕、章节或说话人 review 作为强制交付时可加 `--require subtitles --require chapter_markers --require speaker_turns`。
