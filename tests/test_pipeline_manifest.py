...
def test_generated_motion_window_is_live_verified_and_can_be_required(tmp_path, monkeypatch):
    source = tmp_path / "work" / "generated" / "shot-001.mp4"
    _write(source, "generated bytes")
    media = {
        "duration": 4.0,
        "fps": 24.0,
        "width": 640,
    ...
    monkeypatch.setattr(generated_motion_window, "probe_media", lambda _path: dict(media))
    monkeypatch.setattr(
        generated_motion_window,
        "detect_freezes",
        lambda _path, **_kwargs: [dict(item) for item in freezes],
    )
    plan = generated_motion_window.build_plan(source, project_dir=tmp_path)
    plan_path = tmp_path / "work" / "generated_motion_window.json"
    _write(plan_path, plan)

    pending = build_manifest(str(tmp_path), target_stage="analysis")
    gate = next(g for g in pending["gates"] if g["category"] == "generated_motion_window")
    assert gate["status"] == "blocked"
    assert "generated_motion_window" in pending["blocked_gates"]

    plan = generated_motion_window.confirm_plan(
        plan,
        decision="keep",
        reviewed_by="editor",
        note="The edge holds are intentional and the complete clip was played at 1x.",
    )
    _write(plan_path, plan)
    current = build_manifest(str(tmp_path), target_stage="analysis")
    gate = next(g for g in current["gates"] if g["category"] == "generated_motion_window")
    assert gate["status"] == "warn"
    assert "generated_motion_window" not in current["blocked_gates"]

    _write(source, "changed generated bytes")
    stale = build_manifest(str(tmp_path), target_stage="analysis")
    gate = next(g for g in stale["gates"] if g["category"] == "generated_motion_window")
    assert gate["status"] == "blocked"
    assert "generated_motion_window" in stale["blocked_gates"]

    missing = build_manifest(
        str(tmp_path / "empty"),
        target_stage="analysis",
        required=["generated_motion_window"],
    )
    assert "generated_motion_window" in missing["missing_required"]
