...
`pipeline_manifest.py` 会汇总 transcript、clean_script、render_config、master video、render QA、caption，以及 storyboard/provider/transition 的阻塞状态。`--strict` 在缺少必需 artifact、QA fail、paid approval 或 budget blocker 未解决时返回 2；需要强制字幕或章节交付时，加 `--require subtitles --require chapter_markers`。
