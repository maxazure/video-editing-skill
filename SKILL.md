...
口播音频 + 无声素材
   ...
   ├─→ render_final.py          单次编码渲染（enrich_plan + Heavy 字幕 + 响度规范化）
...
| `--enrich-plan work/enrich_plan.json` | 关 | 自动接入 auto_enrich 的 B-roll / 章节卡 / 贴纸 / 生成图 cue |
...
**6a. 机器质检（先跑）**：
```bash
python3 scripts/render_qa.py final.mp4 --platform douyin --json final_qa.json
```

`render_qa.py` 会检查容器元数据、平台尺寸、视频/音频流、黑屏、长静帧和长静音。对小红书派生文件使用 `--platform xhs`，对抖音/视频号使用 `--platform douyin` 或 `--platform wxch`。如果只想快速查元数据，可加 `--no-filters`。
