...
**6e. 章节 marker 交付（YouTube/B站/课程/metadata 时跑）**：
```bash
...
`pipeline_manifest.py` 会汇总 transcript、clean_script、render_config、master video、render QA、caption，以及 storyboard/provider/transition/motion_guard/speaker_turns/privacy_redaction/localization_pack 的阻塞状态。`--strict` 在缺少必需 artifact、QA fail、paid approval、budget blocker、motion_guard、speaker_turns、privacy_redaction 或 localization_pack blocker 未解决时返回 2；需要强制字幕、章节、说话人回合、视觉隐私或多语交付时，加 `--require subtitles --require chapter_markers --require speaker_turns --require privacy_redaction --require localization_pack`。
